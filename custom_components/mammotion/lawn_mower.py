...
class MammotionLawnMowerEntity(MammotionBaseEntity, LawnMowerEntity):
    ...
    async def async_start_mowing(self) -> None:
        ...
        if self.rpt_dev_status.sys_status == WorkMode.MODE_PAUSE:
            try:
                await self.coordinator.manager.command("resume_execute_task")
        ...
        try:
            await self.coordinator.manager.command("start_job")
    ...
    async def async_dock(self) -> None:
        ...
        try:
            if mode == WorkMode.MODE_RETURNING:
                await self.coordinator.manager.command("cancel_return_to_dock")
                return await self.coordinator.manager.command("get_report_cfg")
            if mode == WorkMode.MODE_WORKING:
                await self.coordinator.manager.command("pause_execute_task")
            await self.coordinator.manager.command("return_to_dock")
            await self.coordinator.async_request_iot_sync()
        except COMMAND_EXCEPTIONS as exc:
            raise HomeAssistantError(
