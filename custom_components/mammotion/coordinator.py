...
class MammotionBaseUpdateCoordinator[DataT](DataUpdateCoordinator[DataT]):  # type: ignore[misc]
    ...
    def _mqtt_transport(self) -> Transport | None:
        """Return the cloud MQTT transport for this device, if any."""
        if handle := self.manager.mower(self.device_name):
            for t_type in (TransportType.CLOUD_ALIYUN, TransportType.CLOUD_MAMMOTION):
    ...
    def mqtt_sends_in_window(self) -> int | None:
        """Outbound MQTT sends in the rolling budget window (currently 12 h / 600).
    ...
    def mqtt_rate_limited(self) -> bool:
        """True while the self-imposed cloud send budget is exhausted."""
        if transport := self._mqtt_transport():
    ...
    def command_timeouts_in_window(self) -> int:
        """Command ACK timeouts in the rolling 24 h window (all coordinators)."""
    ...
    async def _async_update_data(self) -> MowerInfo:
        ...
        try:
            ...
            if not device.map.is_map_synced(bol_hash):
                ...
                skip_while_mowing = not self.config_entry.options.get(
                    CONF_MAP_SYNC_WHILE_MOWING, False
                )
                if skip_while_mowing and sys_status in MOWING_ACTIVE_MODES:
                    LOGGER.debug(
                        "Skipping map sync for %s while mowing (map_sync_while_mowing off)",
                        self.device_name,
                    )
                else:
                    await self.manager.start_map_sync(self.device_name)
